...
change_name_path = "gs://cloocus-swkim-src/컬럼변경.csv"
...
col_name = spark.read\
  ...
  .csv("gs://cloocus-gcs-dp-files/column_change_list.csv")
col_name = col_name.withColumn("OLD_COL_NM", lower(col("OLD_COL_NM")))
...
#   print(dataset_id, table_id)
      ...
      remote_table = (spark.read.format("jdbc") \
        ...
        .load())
      if dataset_id  != 'DAM1':
            for item in checker.collect():
                  if item.OLD_COL_NM in remote_table.columns:
                        remote_table = remote_table.withColumnRenamed(item.OLD_COL_NM ,item.NEW_COL_NM)

      remote_table.createOrReplaceTempView(f"df{idx}")
      remote_table.write.format("parquet").option("path", f"gs://cloocus-gcs-init-full-load/{dataset_id}-{table_id}").save()
